...
> ## STATUS: PROJECT CLOSED — 100% FINISHED (2026-09-09)
>
> The refactor is **complete and shipped**. Phases 0–4 landed in full. Phase 5 shipped
> its canonical-illustration track (5b). Phase 6 shipped the `.table-wrap` scroll
> container, the scroll-shadow affordance, the dark-mode colour audit, and a clean
> link / duplicate-id audit.
>
> Everything still open at that point has been **deliberately descoped and will not be
> built**. This is a personal study guide, not a product: the remaining items—
> theme-aware Plotly redraw logic, `.table-wrap` ARIA labelling, CDN offline/SRI
> fallbacks, the twelve reproducible Matplotlib SVGs of Phase 5a, and the W3C / axe /
> Lighthouse / WCAG / print / offline validation sweeps of Phase 6 — are
> over-engineering for an audience of one. They are struck through below and kept only
> as a record of what was considered and consciously dropped.
>
> **No further work is planned on this repository. Nothing below is a TODO.**
...
> **DESCOPED — will not be built.** Retained below only as a record of what was considered. None of the twelve figures in the table below were generated; the chapters keep their inline-SVG and Plotly figures.
...
> **SHIPPED.** This track is complete. All 12 citation cards are placed across chapters 6–10 (`4f6cd3a`).
...
> **DESCOPED — will not be built.** Retained below only as a record of what was considered. Literal seeds landed in Phase 1. The SRI attributes, the local fallback and the CDN-removal decision are dropped — Plotly stays on the CDN and the book requires a network connection to render its interactive figures.
...
> **SHIPPED.** This track is complete. Items 1–9 and 11 are fixed. **Item 10 (CDN SRI + offline fallback) is DESCOPED** and will not be fixed.
...
> **PARTLY SHIPPED, REMAINDER DESCOPED.** `audit_links.py` reports zero broken links and zero duplicate ids, and the book contains no `<img>` tags so the `alt` requirement is vacuous. The W3C Nu validator pass and the heading-order audit are **descoped and will not be run**.
...
> **PARTLY SHIPPED, REMAINDER DESCOPED.** The `.table-wrap` scroll container, its scroll-shadow affordance and the `.fig` overflow safety net shipped in `3a263e7`/`d91b7a6`; mobile degradation was spot-checked. The seven-width manual sweep, the 320 px math check, the measure audit, the per-breakpoint dark-mode pass and `@media print` are **descoped**.
...
> **DESCOPED — will not be built.** The dark-mode colour audit of `d91b7a6` fixed 15 colours and that is where accessibility work stops. axe/Lighthouse, the full WCAG AA sweep, `prefers-reduced-motion`, `:focus-visible` and the offline `file://` acceptance test are all dropped. **`.table-wrap` will not receive `role`/`aria-label`.** **Plotly will not be made theme-aware.**
...
> **DESCOPED — will not be built.** Retained below only as a record of what was considered. No `README.md`, no `docs/STYLE.md`, no glossary and no single-document build. This plan and `CHEKAI_NAHUI_YEBAT.md` are the project's documentation.
...
- [x] `git init`; `.gitignore` excluding `Statistical Perspectives on Machine Learning Interpretability/`
- [x] Commit all 13 HTML files unmodified as the baseline
- [x] Write `tools/audit_links.py`; record baseline output (expect: 15 cross-file anchors, all resolving)
- [x] Write `tools/inventory.py`; record baseline counts per file
- [x] Move `PROGRESS_study_materials.md`, `revision_log.md` → `docs/legacy/`
- [x] Read `docs/legacy/revision_log.md` lines 259–288 before starting Phase 1

## 3.2 Phase 1 — Shared assets and shell

- [x] `assets/book.css` reconciling all 6 CSS variants into one
- [x] Restore `--safe` / `.badge.safe`; delete all 5 inline `style="background:…"` hacks
- [x] Rename `--trap` → `--pitfall`; add `--practicum`
- [x] Settle `h3` typography (recommend serif, matching h1/h2)
- [x] Replace `body{width:66.67%}` with `max-width:min(72ch, 100% - 3rem)`
- [x] Convert the type scale to rem
- [x] Add dark-mode token overrides + `@media print`
- [x] `assets/book.js`: MathJax config, `XAI.rng(seed)`, namespaced `CFG`/`AXIS`, prev/next, scroll-spy, back-to-top, theme toggle
- [x] Verify `assets/*` load correctly over `file://` (classic `<script src>` / `<link>` only — no ES modules, no `fetch`)
- [x] Define the chapter shell: header / breadcrumb / abstract / TOC / main / chapter-nav / footer
- [x] Rename all 11 chapter files to §1.1 slugs, including the 9 ↔ 10 swap
- [x] Rewrite every internal `href` for the new names; `audit_links.py` → zero failures
- [x] Prefix all 16 inline-SVG class names per figure; verify no two SVGs on one page collide
- [x] Replace all 28 `Math.random()` call sites with seeded `XAI.rng(<literal>)`

## 3.3 Phase 2 — Index and matter

- [x] Rebuild `index.html` on the part/chapter structure of §1.2
- [x] Remove all 12 probability/tier badges and the `Moed A` mention (card 10)
- [x] Replace `.card-lead` + `.card-toc` with `.ch-topics` concept lists — no raw LaTeX
- [x] Add `.ch-practicum` markers to chapters 3, 4, 5, 6, 10
- [x] Write the front-matter "how to read this book" section
- [x] Create `appendix-a-practicums.html` (index of practicums + numerical-verification checklist)
- [x] Create `references.html` (alphabetical, DOI/arXiv links)

## 3.4 Phase 3 — Content refactor

Per chapter (repeat 11×):

- [x] Apply the shared shell; strip the inline `<style>` and duplicated MathJax config
- [x] Add `<p class="chapter-abstract">`
- [x] Convert every `.trap` → `.pitfall`, «Ловушка» → «Частая ошибка» (144 site-wide)
- [x] Convert «Карточки для экзамена» → «Ключевые выводы главы», rewritten as prose
- [x] Insert `section.practicum` blocks per the §1.3 map, **deduplicating** against existing `.calcbox` content
- [x] Verify practicum `id`s do not collide with existing section `id`s
- [x] Add outbound cross-references — priority on chapters that currently have zero (old 05, 06, 07, 08, 10, 11)
- [x] `tools/inventory.py` diff: no `.calcbox`, `.fig` or `$$` block lost in migration

Topic-12-specific:

- [x] t1, t2 → ch. 3 (dedupe against §5/§6)
- [x] t3, t4 → ch. 4 (**pure additions** — ch. 4 has zero calcboxes today)
- [x] t5 → ch. 5 practicum; t6 → merged into ch. 5 §6 prose (not a practicum)
- [x] t7, t8, t9 → ch. 6 (dedupe against §4–5, §7, §10)
- [x] t10 → ch. 10, rewritten as a Ghorbani & Zou case study with all exam/Hebrew references removed
- [x] `#how` + `#checklist` → Appendix A
- [x] Delete `topic-12.html` — **only after** the inventory diff is clean

## 3.5 Phase 4 — Tone purge (grep targets)

Run each as a repo-wide search; expect zero hits on completion:

- [x] `Moed`, `מועד`, `שאלה`, `מבחן`, `XAI_MoedA_2026`, `sample_question`, `final_solution`
- [x] `экзамен`, `на экзамене`, `баллы`, `сдать`, `Страховка`, `Ядро курса`, `Повышенная вероятность`, `Счётные задачи`, `тир`
- [x] `🟢`, `🟡`, `🟠`, `🔴` (badge emoji — also appear inside `<h2>` headings, e.g. «Задача 1 · 🔴 …»)
- [x] `Карточки для экзамена`
- [x] `Ловушка` → «Частая ошибка»
- [x] `ДЗ 2` / `ДЗ 3` / `ДЗ 4` → «упражнение» / neutral problem setup
- [x] `Читательская 1–4` → «Разбор статьи: <Author Year>»
- [x] `не цитировать на экзамене` (in `.fig-cap.illustr::before`) → «Схематическая иллюстрация; числовые значения синтетические.»
- [x] `src: master_conspect.md` / `study_plan.md` / `course_map.md` / `topic_index.md` — all 60+ replaced with real citations into `references.html`
- [x] `Лекция N` / `тема N` in `.meta` → «Часть N · Глава M»
- [x] `Блок N` in `<title>` → new title scheme
- [x] `← Back to topics` → «← Оглавление» (also fixes the current RU/EN mismatch)
- [x] Final human read-through of every chapter opening and closing paragraph — greps do not catch tone

## 3.6 Phase 5 — Visuals

- ~~Scaffold `assets/figures/`, `assets/figures/src/`, `requirements.txt`, regeneration `README.md`~~ — **DESCOPED:** *only `assets/figures/canonical/` was ever needed*
- ~~Port `…/test1.py` as the seed for the ch. 4 PD/M-plot/ALE flagship figure~~ — **DESCOPED:** *Phase 5a not built*
- ~~Produce the 12 reproducible SVGs of §5a; verify each matches its practicum arithmetic digit-for-digit~~ — **DESCOPED:** *Phase 5a not built*
- ~~Every SVG: stable `viewBox`, no fixed width, legible text at 320 px~~ — **DESCOPED:** *no generated SVGs exist*
- [x] Build the `<figure class="canonical">` citation-card component — *shipped in `4f6cd3a`*
- [x] Place all 12 canonical-illustration cards of §5b with correct citations and DOI/arXiv links — *12/12 placed across chs. 6-10*
- [x] Confirm no copyrighted figure is committed or hotlinked; document the drop-in slot — *enforced by `.gitignore` + `assets/figures/canonical/README.md`*
- ~~Remaining Plotly figures: literal seeds, aspect-ratio wrappers, SRI + local fallback~~ — **DESCOPED:** *seeds landed in Phase 1; SRI + local fallback descoped*
- ~~If ≤8 Plotly figures remain, convert them and drop the CDN dependency~~ — **DESCOPED:** *Plotly stays on the CDN*

## 3.7 Phase 6 — QA gate

- ~~W3C Nu validator — zero errors on all 14 pages~~ — **DESCOPED:** *no validation sweep will be run*
- [x] `tools/audit_links.py` — zero unresolved anchors — *0 broken links, verified 2026-09-09*
- [x] Duplicate-`id` scan — clean on every page — *0 duplicates, verified 2026-09-09*
- [x] All `<img>` have `alt`; all `<figure>` have `<figcaption>` — *vacuous — the book uses 0 `<img>` tags; every figure is inline SVG or Plotly*
- ~~One `h1` per page; no skipped heading levels~~ — **DESCOPED:** *part of the dropped validation sweep*
- ~~Manual pass at 320 / 375 / 414 / 768 / 1024 / 1280 / 1920 px~~ — **DESCOPED:** *mobile degradation spot-checked only*
- ~~Zero horizontal body scroll at every width~~ — **DESCOPED:** *the `.fig` overflow safety net shipped instead*
- [x] `.table-wrap` scroll container replaces the destructive `display:block; white-space:nowrap` rule — *shipped in `d91b7a6`*
- ~~Longest display-math blocks verified at 320 px (migrated ALE / fANOVA / Shapley derivations)~~ — **DESCOPED**
- ~~Measure stays 60–75 characters at every breakpoint~~ — **DESCOPED**
- ~~Dark mode verified at every breakpoint, figures included~~ — **DESCOPED:** *colour audit shipped in `d91b7a6`; Plotly theme-awareness descoped*
- ~~`@media print` verified on one full chapter~~ — **DESCOPED**
- ~~axe / Lighthouse on the index + the three heaviest chapters~~ — **DESCOPED**
- ~~WCAG AA contrast on every badge, callout and link~~ — **DESCOPED:** *15 colours fixed in `d91b7a6`; full sweep descoped*
- ~~`prefers-reduced-motion` and `:focus-visible` honored~~ — **DESCOPED**
- ~~**Offline test**: whole book renders from `file://` with the network disabled~~ — **DESCOPED:** *the CDN offline fallback will not be built*

## 3.8 Phase 7 — Release

- ~~`README.md`~~ — **DESCOPED:** *this plan plus the session report are the documentation*
- ~~`docs/STYLE.md` (block vocabulary, citation format, figure conventions)~~ — **DESCOPED:** *no future chapters are planned*
- ~~Optional: `glossary.html` (RU↔EN terms)~~ — **DESCOPED**
- ~~Optional: `tools/build_all.py` single-document print/offline edition~~ — **DESCOPED**

---

# 4. Verification

> **DESCOPED — this end-to-end protocol was never run and will not be.** The only automated check that is actually maintained is `python tools/audit_links.py`, which passes clean (0 broken links, 0 duplicate ids). Everything below is historical.
